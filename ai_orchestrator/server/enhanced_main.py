...
async def serve():
    ...
    logger.debug("🚀 Starting enhanced serve() function")
    load_dotenv()
    ...
    # New: Check if enhanced features should be enabled
    use_enhanced = os.getenv('USE_ENHANCED_CHAT', 'true').lower() == 'true'
    
    logger.debug(f"⚙️  Enhanced configuration loaded:")
    ...
    logger.debug(f"   - Enhanced features: {use_enhanced}")
    ...
    # Add servicer - choose between enhanced and standard
    if use_enhanced:
        ...
        logger.info("🔧 Using Standard ChatHandler")
    ...
    logger.info(f"🤖 Chat mode: {'Enhanced with Agent Tools' if use_enhanced else 'Standard'}")
    ...
    try:
        ...
        if use_enhanced and hasattr(chat_handler, 'cleanup'):
...
def main():
    ...
    try:
        ...
        logger.error(f"💥 Enhanced server failed to start: {e}")
        sys.exit(1)
